...
def normalize_txn_columns(txn: pd.DataFrame) -> pd.DataFrame:
    """
    Map web-API column names to the legacy engine schema.
    Accepts either Line_Total_USD or Line_Total, Unit_Price_USD or Unit_Price.
    """
    df = txn.copy()
    rename = {"Line_Total_USD": "Line_Total", "Unit_Price_USD": "Unit_Price"}
...
def load_and_validate():
    ...
    if neg_stock:
        print(f"    Fixing {neg_stock} negative stock values → 0")
    ...
    if price_inv:
        print(f"    Fixing {price_inv} price inversions → cost × 1.5")
    ...
    if returns:
        print(f"    Dropping {returns} return rows (negative quantity)")
        txn = txn[txn["Quantity"] > 0].copy()

    # Defensive: drop rows with null dates
    null_dates = txn["Transaction_Date"].isna().sum()
    if null_dates:
        print(f"    Dropping {null_dates} rows with null Transaction_Date")
    ...
    if dupes:
        print(f"    Dropped {dupes} duplicate Transaction_ID+Product_ID rows")
...
def compute_rfm(txn: pd.DataFrame) -> pd.DataFrame:
    ...
    txn = normalize_txn_columns(txn)
    if "Line_Total" not in txn.columns:
        txn["Line_Total"] = 0.0
    ...
    def quantile_score(series: pd.Series, ascending: bool = True) -> pd.Series:
        """Score 1–3 by thirds, tolerating ties (falls back to rank)."""
        try:
            labels = [1, 2, 3] if ascending else [3, 2, 1]
            return pd.qcut(series.rank(method="first"), q=3, labels=labels).astype(int)
        except (ValueError, IndexError):
            return pd.Series(2, index=series.index)

    # Score 1–3 using quantile-based bins (equal thirds)
    rfm["R_Score"] = quantile_score(rfm["Recency_Days"], ascending=False)
...
def compute_combo_pairs(txn: pd.DataFrame,
    ...
    txn = normalize_txn_columns(txn)
    has_names = "Product_Name" in txn.columns
    if not has_names:
